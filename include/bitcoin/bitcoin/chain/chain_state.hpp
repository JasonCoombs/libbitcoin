...
public:
    ...
    static uint32_t signal_version(uint32_t forks,  settings& settings);

    /// Create pool state from top chain top block state.
    chain_state(const chain_state& top,  settings& settings);

    /// Create block state from tx pool chain state of same height.
    chain_state(const chain_state& pool,  chain::block& block,
         settings& settings);

    /// Create header state from header pool chain state of parent block.
    chain_state(const chain_state& parent,  chain::header& header,
         settings& settings);
    ...
    chain_state(data&& values, const checkpoints& checkpoints, uint32_t forks,
        uint32_t stale_seconds,  settings& settings);
...
private:
    ...
    static data to_pool(const chain_state& top,  settings& settings);
    static data to_block(const chain_state& pool,  block& block,
        const config::checkpoint& bip9_bit0_active_checkpoint,
        const config::checkpoint& bip9_bit1_active_checkpoint);
    data to_header(const chain_state& parent,  header& header,
         settings& settings);
