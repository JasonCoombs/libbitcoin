...
void extend_data(Target& bytes,  Extension& other)
{
...
byte_array<End - Start> slice( std::array<uint8_t, Size>& bytes)
{
...
byte_array<Left + Right> splice( std::array<uint8_t, Left>& left,
...
byte_array<Left + Middle + Right> splice( std::array<uint8_t, Left>& left,
...
{
    byte_array<Size> out;
    DEBUG_ONLY( auto result =) build_array(out, { bytes });
    BITCOIN_ASSERT(result);
    return out;
}

template <typename Source>
data_chunk to_chunk( Source& bytes)
{
    return data_chunk(std::begin(bytes), std::end(bytes));
}

template <typename Source>
bool starts_with( typename Source::const_iterator& begin,
     typename Source::const_iterator& end,  Source& value)
{
