...
ptree property_list(chain::header& header)
{
    chain::header& block_header = header;

    ptree tree;
    tree.put("bits", block_header.bits());
    tree.put("hash", config::hash256(block_header.hash()));
    tree.put("merkle_tree_hash", config::hash256(block_header.merkle()));
    tree.put("nonce", block_header.nonce());
    tree.put("previous_block_hash", config::hash256(block_header.previous_block_hash()));
    tree.put("time_stamp", block_header.timestamp());
    tree.put("version", block_header.version());
    return tree;
}

ptree property_tree(chain::header& header)
{
    ptree tree;
...
ptree property_tree( std::vector<chain::header>& headers, bool json)
{
    ptree tree;
...
ptree property_list( chain::input& tx_input)
{
    ptree tree;
    ...
    // This will have default versioning, but the address version is unused.
     auto address = tx_input.address();

    if (address)
        tree.put("address_hash", config::hash160(address.hash()));

    tree.put("previous_output.hash", config::hash256(tx_input.previous_output().hash()));
...
ptree property_tree( chain::input& tx_input)
{
    ptree tree;
...
ptree property_tree( chain::input::list& tx_inputs, bool json)
{
    ptree tree;
...
/*
ptree property_list( chain::input& input)
{
     chain::input& tx_input = input;
    return property_list(tx_input);
}

ptree property_tree( chain::input& input)
{
    ptree tree;
...
ptree property_tree( std::vector<chain::input>& inputs, bool json)
{
     auto tx_inputs = cast<input, chain::input>(inputs);
...
ptree property_list( chain::output& tx_output)
{
    ptree tree;

    // This does not support pay_multisig or pay_public_key (nonstandard).
    // This will have default versioning, but the address version is unused.
     auto address = tx_output.address();
...
ptree property_tree( chain::output& tx_output)
{
    ptree tree;
...
ptree property_tree( chain::output::list& tx_outputs, bool json)
{
    ptree tree;
...
ptree property_list( chain::point_value& point)
{
    ptree tree;
    tree.put("hash", config::hash256(point.hash()));
    tree.put("index", point.index());
    tree.put("value", point.value());
    return tree;
}

ptree property_tree( chain::points_value& values, bool json)
{
    ptree tree;
...
ptree property_list(chain::transaction& transaction, bool json)
{
    chain::transaction& tx = transaction;

    ptree tree;
    tree.put("hash", config::hash256(tx.hash()));
...
ptree property_tree(chain::transaction& transaction, bool json)
{
    ptree tree;
...
ptree property_tree( std::vector<chain::transaction>& transactions, bool json)
{
    ptree tree;
...
ptree property_list( wallet::wrapped_data& wrapper)
{
    ptree tree;
    tree.put("checksum", wrapper.checksum);
    tree.put("payload", config::base16(wrapper.payload));
    tree.put("version", wrapper.version);
    return tree;
}

ptree property_tree( wallet::wrapped_data& wrapper)
{
    ptree tree;
...
ptree property_list( hash_digest& hash, size_t height, size_t index)
{
    ptree tree;
    tree.put("hash", config::hash256(hash));
    tree.put("height", height);
    tree.put("index", index);
    return tree;
}

ptree property_tree( hash_digest& hash, size_t height, size_t index)
{
    ptree tree;
...
ptree property_tree( settings_list& settings)
{
    ptree list;
...
ptree property_tree( std::error_code& code, uint32_t sequence)
{
    ptree tree;
    tree.put("sequence", sequence);
    tree.put("type", "error");
